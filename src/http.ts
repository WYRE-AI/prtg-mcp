...
function startHttpServer(): void {
  ...
  const httpServer = createHttpServer(async (req, res) => {
    ...
    if (url.pathname === '/health') {
      ...
      // mode, credentials arrive per-request via X-PRTG-Api-Key/
      // X-PRTG-Base-URL, not at startup - checking getCredentials() here
    ...
    const apiKey = isGatewayMode ? (req.headers['x-prtg-api-key'] as string | undefined) : undefined;
    const baseUrl = isGatewayMode ? (req.headers['x-prtg-base-url'] as string | undefined) : undefined;
